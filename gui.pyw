...
TITLE = 'Telegram Message Tracker'
SIZE = '640x280'
...
class App(tkinter.Tk):
    ...
    def __init__(self, client, *args, **kwargs):
        ...
        self.chat.insert(tkinter.END, 'https://t.me/letradingfrancais')
        ...
        self.chat.focus()
        self.chat_id = None
        self.sel = IntVar(value=1)

        # Message log (incoming and outgoing); we configure it as readonly
        self.log = tkinter.scrolledtext.ScrolledText(self)
        allow_copy(self.log)
        self.log.grid(row=4, column=0, columnspan=3, sticky=tkinter.NSEW)
        self.rowconfigure(4, weight=1)
        self.cl.add_event_handler(self.on_new_message, events.NewMessage)
        ...
        # Select Output mode
        self.selrealtime = tkinter.Radiobutton(self, text="In Real time", variable = self.sel, value=1)
        self.selrealtime.grid(row=3, column=0)
        
        self.selhistory = tkinter.Radiobutton(self, text="History", variable=self.sel, value=2)
        self.selhistory.grid(row=3, column=1)
        self.export_btn = tkinter.Button(self, text='Export',
                       command=self.export_to_csv)
        self.export_btn.grid(row=3, column=2)

        # Open Filename Dialog
        tkinter.Label(self, text='Path:').grid(row=2, column=0)
        BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        self.filename = BASE_DIR + '/result.csv'
        self.path = tkinter.Label(self, text=self.filename)
        self.path.grid(row=2, column=1)
        tkinter.Button(self, text='Browse',
                       command=self.open_file_dialog).grid(row=2, column=2)
    ...
    async def on_new_message(self, event):
        ...
        if self.isStart == True: 
            with open(self.filename, '+a', encoding='UTF8') as f:
                ...
                printData = [utils.get_display_name(sender) + ';' + event.raw_text.replace(' ', ';') + ';' + str(datetime.datetime.now())]
                self.csv_writer.writerow(printData)
    ...
    async def on_message(self, event):
        ...
        text = text.replace(' ', ';')
        with open(self.filename, '+a', encoding='UTF8') as f:
            printData = [text + ';' + str(datetime.datetime.now())]
            self.csv_writer = csv.writer(f)
            self.csv_writer.writerow(printData)
    ...
    @callback
    async def open_file_dialog(self, event=None):
        filename = fd.askopenfilename(title='Save to', initialdir='/', filetypes=(('csv files', '*.csv'), ('All files', '*.*')))
        self.filename = filename
        self.path.configure(text=filename)
    ...
    @callback
    async def export_to_csv(self, event=None):
        chat = self.chat.get().strip()
        ...
        if self.sel.get() == 1:
            self.isStart = True
            self.log.delete('1.0', tkinter.END)
            self.log.insert(tkinter.END, '---- Real time Message saving started ---- \n')
            self.export_btn.configure(text="Stop", command=self.stop_export)
        else:
            self.log.delete('1.0', tkinter.END)
            self.log.insert(tkinter.END, '---- History Message saving  started ---- \n')
            await self.check_chat()
            self.log.insert(tkinter.END, '---- History Message saving  Ended ---- \n')

    @callback
    async def stop_export(self, event=None):
        self.isStart = False
        self.export_btn.configure(text = 'Export', command=self.export_to_csv)
        self.log.insert(tkinter.END, "---- Real time Message saving ended ---- \n")
